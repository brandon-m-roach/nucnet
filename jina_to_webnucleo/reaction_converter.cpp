...
)
{
  ...
  {
    boost::heap::fibonacci_heap<unsigned int> z_reac;
    boost::heap::fibonacci_heap<unsigned int> z_prod;
...
//--------------------------------------------------
// AS - check the charge conservation

    std::vector<unsigned int> z_reac;
    std::vector<unsigned int> z_prod;
	int zReac=0;
	int zProd=0;
	
    for( size_t i = 0; i < reactants.size(); i++ )
    {
		if(reactants[i]!="electron" && reactants[i]!="positron" && reactants[i]!="neutrino_e" && reactants[i]!="anti-neutrino_e" && reactants[i]!="gamma"){
      		z_reac.push_back(
        		Libnucnet__Species__getZ(
          		Libnucnet__Nuc__getSpeciesByName( p_nuc, reactants[i].c_str() )
        		)
		...
		else if(reactants[i]=="electron"){
			zReac--;
    	}
		else if(reactants[i]=="positron"){
			zReac++;
    	}
	}
    for( size_t i = 0; i < products.size(); i++ )
    {
		if(products[i]!="electron" && products[i]!="positron" && products[i]!="neutrino_e" && products[i]!="anti-neutrino_e" && products[i]!="gamma"){
      		z_prod.push_back(
        		Libnucnet__Species__getZ(
          		Libnucnet__Nuc__getSpeciesByName( p_nuc, products[i].c_str() )
        		)
		...
		else if(products[i]=="electron"){
			zProd--;
    	}
		else if(products[i]=="positron"){
			zProd++;
    	}
	...
	if ((zReac - zProd) !=0) {
		std::cout << "Charge is not conserved for this reaction! ";
 		for( size_t i = 0; i < i_reac; i++ )
  		{
			std::cout << elements[i] << " " ;
  		}
		std::cout << "-> ";
 		for( size_t i = i_reac; i < i_total; i++ )
  		{
			std::cout << elements[i] << " \n" ;
  		}
			std::cout << "Total charge for reactants: " << zReac << " and for products: " << zProd << std::endl;
	} 
//--------------------------------------------------
